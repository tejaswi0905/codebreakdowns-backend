...
import { heartbeatSchema, toggleCompleteSchema, toggleBookmarkSchema, } from "./progress.schemas.js";
import * as progressService from "./progress.trakingService.js";
export const recordHeartbeat = catchAsync(async (req, res) => {
    ...
    const progress = await progressService.processHeartbeat(userId, lessonId, currentTime);
    sendSuccess(res, 200, progress, "Heartbeat recorded.");
});
export const toggleComplete = catchAsync(async (req, res) => {
    const userId = req.user.id;
    ...
    const progress = await progressService.processToggleComplete(userId, lessonId, isCompleted);
...
export const toggleBookmark = catchAsync(async (req, res) => {
    ...
    const progress = await progressService.processToggleBookmark(userId, lessonId, isBookmarked);
